...
ggplot(data = hate_crimes, aes(x = trump_support, y = hate_crimes)) +
  ...
  labs(
    title = "Hate Crimes per 100K by Level of Trump Support",
    x = "Level of TRump Support in State",
    y = "Hate Crimes per 100K Individuals"
  ) +
  theme_bw()
...
-   Yes I expected that people with **low trump support** would commit more **hate crimes**.
:::
...
get_regression_table(hate_mod) -> P3
kable(P3)
```
:::
...
-   The intercept represents the estimated average number of hate crimes per 100K people in states with **"high"** Trump support.
:::
...
-   The model estimates the number of hate crimes per 100k people with low Trump support will be 0.460.  **Estimate = 0.191 + 0.269 = 0.460**.

```{r}
# Type your code and comments inside the code chunk
P3$estimate[1] + P3$estimate[2]
```
:::
...
```{r}
# Type your code and comments inside the code chunk
high_support <- coef(hate_mod)["(Intercept)"]
low_support <- coef(hate_mod)["(Intercept)"] + coef(hate_mod)["trump_supportlow"]

cat("High Support Estimate:", high_support, "\n")
cat("Low Support Estimate:", low_support, "\n")

low_support > high_support
```

-   The model estimates that states with **"low"** Trump support will have more frequent hate crimes.
:::
...
```{r, label = "delta"}
# Type your code and comments inside the code chunk
difference_medium_high <- coef(hate_mod)["trump_supportmedium"]

print(difference_medium_high)
```

-   States with medium Trump support had 0.0314 greater hate crimes per 100K people than states with high Trump support .
:::
...
```{r}
# Type your code and comments inside the code chunk
regression_points <-get_regression_points(hate_mod)
regression_points %>%
  distinct(trump_support, hate_crimes_hat)
```

-   The Y fitted value for low is **0.460**.

-   The Y fitted value for medium is **0.222**.

-   The Y fitted value for high is **0.191**.
:::
...
-   The group means calculated are **exactly identical** to the three possible fitted Y values.
:::
...
$$\hat{y} = 0.191 + 0.2691111 \times 0 + 0.314 \times 0 = 0.191$$
...
y_hat_high <- intercept + (beta_low * D1_low) + (beta_med * D2_med)

print(y_hat_high)
```

-   For states with high Trump support the estimated hate crime per 100K people is 0.191 which is our intercept for our regression equation.
:::
...
-   The five states with the highest rate of hate crimes are Oregon, Washington, Massachusetts, Minnesota, and Maine. They also all had **LOW** Trump support. This did not surpise me sice these states are very democratic.
:::
...
ggplot(data = hate_crimes, aes(x = unemployment, y = hate_crimes)) +
  ...
  labs(
    title = "Hate Crimes per 100K by State Unemployment Level",
    x = "Unemployment Level (low vs. High)",
    y = "Hate Crimes per 100K Individuals"
  ) +
  theme_bw()
```
...
$$\widehat{\text{hate\_crimes}} = b_0 + b_1 \times \text{unemployment}_{\text{high}}$$.
:::
...
-   The estimate of the intercept of the table is **0.289** .
:::
...
```{r}
# Type your code and comments inside the code chunk
new_state <- data.frame(unemployment = "low")
predict(job_mod, newdata = new_state)
```

-   The model estimates that the the number of hate crimes per 100K people with low unemployment will be **0.320087** .
:::
...
-   The two possible fitted values are the intercept of 0.289 and the estimated value of low unemployment at 0.320.
:::
...
-  A residual represents the error of the model's prediction, it is calculated using **RESIDUAL = OBSERVED - FITTED.
:::
...
-   There were more hate crimes in areas with low median household incomes. Since the estimate is negative it tells us there is a inverse relationship so as income goes up, hate crimes go down.
:::
...
```{r}
# Type your code and comments inside the code chunk
get_regression_points(job_med_mod) %>% head(6)
```
:::
...
-   hate_crimes which is the actual observed value is 0.616. hate_crimes_hat which is the predicted value is 0.229. The residual which is the prediction error is actual - predicted which is 0.387. Since the residual is positive that means the model **under predicted** the hate crime level. But if the residual was negative than the model would've overestimated the hate crime level.
:::
